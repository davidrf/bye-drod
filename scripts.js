...
function bombsAway() {
  ...
  setTimeout(() => {
    ignition.className = ignition.className.slice(
      0,
      ignition.className.length - ' bombsAway'.length,
    );
    partyBoat.className += ' vroom';
    setTimeout(() => {
      partyBoat.className = partyBoat.className.slice(
        0,
        partyBoat.className.length - ' vroom'.length,
      );
      byebyeBoat();
    }, 3000);
  }, 1000);
